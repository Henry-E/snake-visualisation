$(document).ready(function(){ 
  ...
  var Agent = function() {
   ...
   backward: function() {
    ...
    // Don't care about checking for collisions or negative reward anymore
    ...
    // last_step_array = zeros([w/cw, h/cw]);
        ...
        create_snake();
        // overwrites existing food variable with list of array points.
        food = [];
        food = [{x:4, y:0}, {x:3, y:0}, {x:3, y:1}, {x:2, y:1}, {x:2, y:2}, {x:1, y:2}, {x:1, y:3}, {x:0, y:3}, {x:4, y:3}, {x:4, y:4}];
        // food.push({x:-1, y:-1});
    ...
    //     // no longer care about choosing overlapping points, going with the centre for now.
    //     // need to work on allowing the for the food variable containing an array instead
    //     // it should theoretically only apply for visualising? Oh, also for checking if the
    //     // agent is currently over a food item.
    //     food = {
    //         x: 0, 
    //         y: 0, 
    //     };
    ...
    function paint()
    {
        ...
        {
            input_array = input_array.concat(current_step_array[i]);
        }
        ...
        // We don't care about running the check on collisions anymore
        ...
        // We're not worrying anymore about matching the position of the head with that of the food.
        // It only matters, that we take the last cell and place it at the location of the next move.
        var tail = snake_array.pop(); //pops out the last cell
        tail.x = nx; tail.y = ny;
    ...
    function get_input_array() {
        ...
        for(var i = 0; i < food.length; i++)
        {
            test_array[food[i].x][food[i].y] = 1;
        }


        // test_array[food.y][food.x] = 1;

        // We're going to start allowing the snake to overlay
        // the food item location, which might confuse the
        // neural net less? Also limiting the array length
        // to one, so that only the head of the snake is used now
        // again, hopefully less confusion.
